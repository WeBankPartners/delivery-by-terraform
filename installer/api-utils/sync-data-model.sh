...
RESPONSE_JSON=$(curl -sSfL \
	--request GET "http://${CORE_HOST}:8080/platform/v1/models/package/${PLUGIN_PACKAGE_NAME}" \
...
#	--request OPTIONS "http://${CORE_HOST}:8080/platform/v1/models" \
#	--header "Authorization: Bearer ${ACCESS_TOKEN}" \
#	| grep 'Allow:'
#)
#if [ "${ALLOWED_METHODS/POST/}" != "${ALLOWED_METHODS}" ]; then
#	curl -sSfL --request POST "http://${CORE_HOST}:8080/platform/v1/models" \
#		--header "Authorization: Bearer ${ACCESS_TOKEN}" \
