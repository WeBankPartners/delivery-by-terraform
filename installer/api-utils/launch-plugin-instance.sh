...
INSTANCE_HOST=$(curl -sSfL \
	--request GET "http://${CORE_HOST}:8080/platform/v1/available-container-hosts" \
...
INSTANCE_PORT=$(curl -sSfL \
	--request GET "http://${CORE_HOST}:8080/platform/v1/hosts/${INSTANCE_HOST}/next-available-port" \
	--header "Authorization: Bearer ${ACCESS_TOKEN}" \
	| ${SCRIPT_DIR}/check-status-in-json.sh \
