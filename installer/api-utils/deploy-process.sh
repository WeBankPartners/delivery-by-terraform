...
#	--request GET "http://${CORE_HOST}:8080/platform/v1/users/roles" \
...
PROCESS_DEF_ID=$(curl -sSfL \
	--request POST "http://${CORE_HOST}:8080/platform/v1/process/definitions/import" \
	--header "Authorization: Bearer ${ACCESS_TOKEN}" \
...
curl -sSfL \
	--request POST "http://${CORE_HOST}:8080/platform/v1/process/definitions/deploy/${PROCESS_DEF_ID}" \
	--header "Authorization: Bearer ${ACCESS_TOKEN}" \
